...
export default function App() {
  // State untuk testing routing sederhana
  const [currentPage, setCurrentPage] = useState('home');
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  
  // Simulasi jumlah item di keranjang
  const cartLength = 0; 
  ...
  return (
    <div className="font-sans text-gray-800 bg-gray-50 relative">
      ...
      {/* --- KODE INI NANTI BISA KAMU COPY KE src/components/Navbar.jsx --- */}
      <nav className="sticky top-0 z-50 bg-white shadow-sm border-b border-gray-100">
        <div className="container px-4 py-3 md:py-4 mx-auto flex justify-between items-center max-w-7xl">
          ...
          <div className="hidden md:flex items-center">
            ...
            <div className="flex space-x-8 font-medium text-[15px] mr-6">
              <button 
                ...
                className={`transition ${currentPage === 'home' ? 'font-bold text-[#0B7A3E]' : 'text-[#0B7A3E] hover:opacity-70'}`}
              >
                Beranda
              </button>
              <button 
                onClick={() => handleNav('tour')} 
                className={`transition ${currentPage === 'tour' ? 'font-bold text-[#0B7A3E]' : 'text-[#0B7A3E] hover:opacity-70'}`}
              >
                Tour
              </button>
              <button 
                onClick={() => handleNav('sewa')} 
                className={`transition ${currentPage === 'sewa' ? 'font-bold text-[#0B7A3E]' : 'text-[#0B7A3E] hover:opacity-70'}`}
              >
                Sewa Mobil
              </button>
              <button 
                onClick={() => handleNav('driver')} 
                className={`transition ${currentPage === 'driver' ? 'font-bold text-[#0B7A3E]' : 'text-[#0B7A3E] hover:opacity-70'}`}
              >
                Driver
              </button>
            </div>
            ...
            <button 
              ...
              className="ml-4 relative flex items-center gap-2 bg-[#E8F5E9] text-[#0B7A3E] px-6 py-2.5 rounded-full font-bold hover:bg-[#d1ebd6] transition shadow-sm"
          ...
          <div className="flex md:hidden items-center gap-3">
             <button 
                onClick={() => handleNav('cart')} 
                className="relative flex items-center justify-center bg-[#E8F5E9] text-[#0B7A3E] p-2 rounded-full"
             >
                <ShoppingCart size={20} />
                {cartLength > 0 && (
